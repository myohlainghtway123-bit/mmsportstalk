...
export default function PredictionScreenV2({ openMatch, openAccount }) {
  ...
  return <View style={s.screen}>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>load(true)} colors={[C.red]} tintColor={C.red}/>} showsVerticalScrollIndicator={false}>
      ...
      {tab === "Points" ? <>
        <View style={s.statsGrid}><View style={s.stat}><Text style={s.statLabel}>POINTS</Text><Text style={s.statNumber}>{auth ? totals.points : "—"}</Text></View><View style={s.stat}><Text style={s.statLabel}>EXACT SCORES</Text><Text style={s.statNumber}>{auth ? totals.exact : "—"}</Text></View><View style={s.stat}><Text style={s.statLabel}>RANK</Text><Text style={s.statNumber}>{auth && myRank ? `#${myRank}` : "—"}</Text></View></View>
        <View style={s.pointsInfo}><Ionicons name="information-circle-outline" size={20} color={C.red}/><Text style={s.pointsInfoText}>Your MST Score points and rank use the same prediction system as myanmarsportstalk.com.</Text></View>
      </> : null}
      ...
      {tab === "Leaderboard" ? <>
        ...
        <View style={s.leaderCard}><View style={[s.leaderRow,s.leaderHeader]}><Text style={[s.leaderCell,{width:28}]}>#</Text><Text style={[s.leaderCell,{flex:1}]}>PLAYER</Text><Text style={s.leaderCell}>PTS</Text><Text style={s.leaderCell}>EXACT</Text><Text style={s.leaderCell}>CORRECT</Text><Text style={s.leaderCell}>PLAYED</Text></View>{leaders.length ? leaders.map((x,i)=><View key={`${x.id}-${i}`} style={[s.leaderRow,i!==leaders.length-1&&s.divider]}><Text style={[s.leaderRank,{width:28},Number(x.rank)<=3&&{color:C.gold}]}>{x.rank}</Text><Text numberOfLines={1} style={[s.leaderName,{flex:1}]}>{x.name}</Text><Text style={s.leaderValue}>{x.points}</Text><Text style={s.leaderValue}>{x.exact}</Text><Text style={s.leaderValue}>{x.correct}</Text><Text style={s.leaderValue}>{x.played}</Text></View>) : <Text style={s.emptyText}>Leaderboard is empty right now.</Text>}</View>
      </> : null}
    </ScrollView>
  </View>;
}
...
const s = StyleSheet.create({
  ...
  statsGrid:{flexDirection:"row",gap:8,marginVertical:8},stat:{flex:1,minHeight:75,backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:10,padding:10,justifyContent:"space-between"},statLabel:{fontSize:8.8,fontWeight:"900",color:C.muted},statNumber:{fontSize:22,fontWeight:"900",color:C.text},historyCard:{backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:11,overflow:"hidden"},historyRow:{padding:11},historyTeams:{flexDirection:"row",alignItems:"center",gap:8},historyTeam:{flex:1,flexDirection:"row",alignItems:"center",gap:7},historyName:{flex:1,fontSize:10.5,fontWeight:"700",color:C.text2},historyScore:{fontSize:15,fontWeight:"900",color:C.text,minWidth:54,textAlign:"center"},historyMeta:{marginTop:8,flexDirection:"row",justifyContent:"flex-end",alignItems:"center",gap:8},historyStatus:{fontSize:9.4,color:C.muted},pointsBadge:{minWidth:44,height:25,borderRadius:13,backgroundColor:C.card2,alignItems:"center",justifyContent:"center",paddingHorizontal:8},pointsText:{fontSize:9.5,fontWeight:"900",color:C.text},divider:{borderBottomWidth:1,borderBottomColor:C.border2},pointsInfo:{backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:11,padding:12,flexDirection:"row",alignItems:"center",gap:9},pointsInfoText:{flex:1,fontSize:10,lineHeight:15,color:C.text2},
  leaderIntro:{flexDirection:"row",alignItems:"center",justifyContent:"space-between",marginBottom:5},leaderIntroText:{fontSize:9.3,color:C.muted,marginTop:-4},leaderCard:{backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:11,overflow:"hidden"},leaderRow:{minHeight:45,paddingHorizontal:9,flexDirection:"row",alignItems:"center",gap:5},leaderHeader:{backgroundColor:"#151515"},leaderCell:{width:46,fontSize:7.7,fontWeight:"900",color:C.muted},leaderRank:{fontSize:10.5,fontWeight:"800",color:C.text2},leaderName:{fontSize:10.5,fontWeight:"700",color:C.text2},leaderValue:{width:46,fontSize:10,fontWeight:"800",color:C.text2,textAlign:"left"}
});
