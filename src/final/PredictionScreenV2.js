...
const TABS = ["Predict", "My Predictions", "Points / Ranking"];
...
function HistoryRow({ item }) {
  ...
  const homeName = match?.home?.name || match?.teams?.home?.name || item.raw?.homeTeam || "Home";
  const awayName = match?.away?.name || match?.teams?.away?.name || item.raw?.awayTeam || "Away";
  ...
  return <View style={s.historyRow}>
    ...
    <View style={s.historyMeta}><Text style={s.historyStatus}>{finalReady ? `Final ${item.finalHomeScore} : ${item.finalAwayScore}` : "Pending"}</Text><View style={[s.pointsBadge,item.points===3 && {backgroundColor:C.green},item.points===1 && {backgroundColor:"#5C4B1F"}]}><Text style={s.pointsText}>{finalReady ? `${item.points} pts` : "—"}</Text></View></View>
  </View>;
}
...
export default function PredictionScreenV2({ openMatch, openAccount }) {
  ...
  const load = useCallback(async (refresh=false) => {
    ...
    try {
      ...
      const authStatus = await getAuthStatus().catch(()=>({authenticated:false}));
      ...
      if (authStatus.authenticated) {
        const [predPayload,leaderPayload] = await Promise.all([getAccountPredictions().catch(()=>null),getLeaderboard().catch(()=>null)]);
        setHistory(normalizePredictionPayload(predPayload));
        setLeaders(normalizeLeaderboard(leaderPayload));
      } else {
        setHistory([]); setLeaders([]);
      }
  ...
  const totals = useMemo(()=>({
    ...
    exact: history.filter((x)=>x.exact || x.points===3).length,
    settled: history.filter((x)=>x.finalHomeScore!==null&&x.finalHomeScore!==undefined&&x.finalAwayScore!==null&&x.finalAwayScore!==undefined).length,
  }),[history]);
  ...
  return <View style={s.screen}>
    <View style={s.header}><View><Text style={s.title}>Predictions</Text><Text style={s.subtitle}>Predict scores · earn points · climb the table</Text></View><Ionicons name="trophy-outline" size={27} color={C.text}/></View>
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>load(true)} colors={[C.red]} tintColor={C.red}/>} showsVerticalScrollIndicator={false}>
      {!auth ? <Pressable style={s.signInCard} onPress={openAccount}><Ionicons name="person-circle-outline" size={27} color={C.muted}/><View style={{flex:1}}><Text style={s.signInTitle}>Sign in to save predictions</Text><Text style={s.signInText}>The app uses the same MST account and prediction history as the website.</Text></View><Ionicons name="chevron-forward" size={19} color={C.muted}/></Pressable> : null}
      <View style={s.scoring}><Text style={s.scoringLabel}>SCORING</Text><Text style={s.scoringText}>Exact score: {PREDICTION_SCORING.exact} points · Correct win/draw/loss: {PREDICTION_SCORING.correctOutcome} point · Wrong outcome: {PREDICTION_SCORING.wrong} points.</Text></View>
      ...
      {tab === "My Predictions" ? <>
        <View style={s.statsGrid}><View style={s.stat}><Text style={s.statLabel}>POINTS</Text><Text style={s.statNumber}>{totals.points}</Text></View><View style={s.stat}><Text style={s.statLabel}>EXACT</Text><Text style={s.statNumber}>{totals.exact}</Text></View><View style={s.stat}><Text style={s.statLabel}>SETTLED</Text><Text style={s.statNumber}>{totals.settled}</Text></View></View>
        <Text style={s.sectionTitle}>PREDICTION HISTORY</Text>
        <View style={s.historyCard}>{history.length ? history.map((item,index)=><View key={`${item.id}-${index}`} style={index!==history.length-1&&s.divider}><HistoryRow item={item}/></View>) : <Text style={s.emptyText}>No predictions yet.</Text>}</View>
      </> : null}

      {tab === "Points / Ranking" ? <>
        <View style={s.statsGrid}><View style={s.stat}><Text style={s.statLabel}>POINTS</Text><Text style={s.statNumber}>{totals.points}</Text></View><View style={s.stat}><Text style={s.statLabel}>EXACT SCORES</Text><Text style={s.statNumber}>{totals.exact}</Text></View><View style={s.stat}><Text style={s.statLabel}>RANK</Text><Text style={s.statNumber}>{myRank ? `#${myRank}` : "—"}</Text></View></View>
        <Text style={s.sectionTitle}>TOP PREDICTORS</Text>
        <View style={s.leaderCard}><View style={[s.leaderRow,s.leaderHeader]}><Text style={[s.leaderCell,{width:28}]}>#</Text><Text style={[s.leaderCell,{flex:1}]}>PLAYER</Text><Text style={s.leaderCell}>PTS</Text><Text style={s.leaderCell}>EXACT</Text><Text style={s.leaderCell}>CORRECT</Text><Text style={s.leaderCell}>PLAYED</Text></View>{leaders.map((x,i)=><View key={`${x.id}-${i}`} style={[s.leaderRow,i!==leaders.length-1&&s.divider]}><Text style={[s.leaderRank,{width:28}]}>{x.rank}</Text><Text numberOfLines={1} style={[s.leaderName,{flex:1}]}>{x.name}</Text><Text style={s.leaderValue}>{x.points}</Text><Text style={s.leaderValue}>{x.exact}</Text><Text style={s.leaderValue}>{x.correct}</Text><Text style={s.leaderValue}>{x.played}</Text></View>)}</View>
      </> : null}
    </ScrollView>
  </View>;
}
...
const s = StyleSheet.create({
  screen:{flex:1,backgroundColor:C.bg},header:{minHeight:70,paddingHorizontal:18,flexDirection:"row",alignItems:"center",justifyContent:"space-between",borderBottomWidth:1,borderBottomColor:C.border2},title:{fontSize:22,fontWeight:"900",color:C.text},subtitle:{fontSize:10.5,color:C.muted,marginTop:3},tabs:{height:51,flexDirection:"row",padding:7,gap:6,borderBottomWidth:1,borderBottomColor:C.border2},tab:{flex:1,borderRadius:9,alignItems:"center",justifyContent:"center"},tabOn:{backgroundColor:C.redSoft},tabText:{fontSize:9.6,fontWeight:"800",color:C.muted,textAlign:"center"},tabTextOn:{color:C.red},content:{padding:14,paddingBottom:38},
  ...
  statsGrid:{flexDirection:"row",gap:8,marginVertical:8},stat:{flex:1,minHeight:75,backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:10,padding:10,justifyContent:"space-between"},statLabel:{fontSize:8.8,fontWeight:"900",color:C.muted},statNumber:{fontSize:22,fontWeight:"900",color:C.text},historyCard:{backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:11,overflow:"hidden"},historyRow:{padding:11},historyTeams:{flexDirection:"row",alignItems:"center",gap:8},historyTeam:{flex:1,flexDirection:"row",alignItems:"center",gap:7},historyName:{flex:1,fontSize:10.5,fontWeight:"700",color:C.text2},historyScore:{fontSize:15,fontWeight:"900",color:C.text,minWidth:54,textAlign:"center"},historyMeta:{marginTop:8,flexDirection:"row",justifyContent:"flex-end",alignItems:"center",gap:8},historyStatus:{fontSize:9.4,color:C.muted},pointsBadge:{minWidth:44,height:25,borderRadius:13,backgroundColor:C.card2,alignItems:"center",justifyContent:"center",paddingHorizontal:8},pointsText:{fontSize:9.5,fontWeight:"900",color:C.text},divider:{borderBottomWidth:1,borderBottomColor:C.border2},
  leaderCard:{backgroundColor:C.card,borderWidth:1,borderColor:C.border2,borderRadius:11,overflow:"hidden"},leaderRow:{minHeight:45,paddingHorizontal:9,flexDirection:"row",alignItems:"center",gap:5},leaderHeader:{backgroundColor:"#151515"},leaderCell:{width:46,fontSize:7.7,fontWeight:"900",color:C.muted},leaderRank:{fontSize:10.5,fontWeight:"800",color:C.text2},leaderName:{fontSize:10.5,fontWeight:"700",color:C.text2},leaderValue:{width:46,fontSize:10,fontWeight:"800",color:C.text2,textAlign:"left"}
});
