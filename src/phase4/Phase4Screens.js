    ...
    <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
      ...
      <View style={s.card}>
        ...
        <View style={s.settingsRow}>
          ...
          <View style={{flex:1}}><Text style={s.rowTitle}>MST Score</Text><Text style={s.rowSub}>{process.env.EXPO_PUBLIC_MST_ENVIRONMENT === "production" ? "Version 1.5.2" : "Version 1.5.2 · Android Build 12 (Closed Beta)"}</Text></View>
        </View>
      </View>
    </ScrollView>
  </View>;
...
export function AboutScreen({ goBack }) {
  return <View style={s.screen}><Header title="About MST Score" subtitle="Myanmar Sports Talk" goBack={goBack}/><ScrollView contentContainerStyle={s.content}><View style={s.aboutHero}><Text style={s.mst}>MST</Text><Text style={s.aboutTitle}>{process.env.EXPO_PUBLIC_MST_ENVIRONMENT === "production" ? "MST SCORE · VERSION 1.5.2" : "MST SCORE · VERSION 1.5.2 (CLOSED BETA)"}</Text><Text style={s.aboutText}>Myanmar Sports Talk's premier football application for live scores, in-depth match stats, verified news, transfer updates, score predictions, and qualified Tipsters.</Text></View><Pressable style={s.redButton} onPress={() => Linking.openURL(MST_SITE_URL)}><Text style={s.redButtonText}>OPEN MYANMARSPORTSTALK.COM</Text></Pressable></ScrollView></View>;
}
